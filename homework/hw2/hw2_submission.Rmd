...
In the prior gamma(a, b), the parameter b approximates the prior sample size $s_0$. This is because the gamma prior for lambda when we have $\textbf{Y} = [y_1, y_2, ... y_n]$, $b = b_0 + n$. 
...
***Prior 1***
$$\lambda_5 \sim Gamma(5*3*\frac{49}{9}, \frac{1}{5}*\frac{7}{9}) = Gamma(81.667,0.1556)$$

***Prior 2***
$$\lambda_5 \sim Gamma(5*3*7, 3*1) = Gamma(105, 3)$$

### 2c: Give the two resulting posteriors for lambda5 .

***Posterior 1***
$$\lambda_5 \sim Gamma(5*3*\frac{49}{9} + 19, 3*\frac{7}{9} + 1) = Gamma(100.67,3.3333)$$

***Posterior 2***
$$\lambda_5 \sim Gamma(5*3*7 + 19, \frac{1}{5}*1+1) = Gamma(124, 1.2)$$
...
```{r, echo = F}
posterior1a = c(alpha1p, beta1p, alpha1p/beta1p, alpha1p/beta1p^2)
posterior1b = c(alpha2p, beta2p, alpha2p/beta2p, alpha2p/beta2p^2)
posterior5a = c(alpha5.1p, beta5.1p, alpha5.1p/beta5.1p, alpha5.1p/beta5.1p^2)
posterior5b = c(alpha5.2p, beta5.2p, alpha5.2p/beta5.2p, alpha5.2p/beta5.2p^2)

rbind(posterior1a, posterior1b, posterior5a, posterior5b)
...
dev.new()
plot(density(rgamma(1000, alpha5.1p, beta5.1p)), main = "Posterior and Prior 1 for Lambda5", xlim = c(0, 70))
lines(density(rgamma(1000, alpha5.1, beta5.1)),lty = 2, col = "red")
mean(rgamma(1000, alpha5.1p, beta5.1p))

dev.new()
plot(density(rgamma(1000, alpha5.2p, beta5.2p)), xlim = c(0, 70), main = "Posterior and Prior 2 for Lambda5")
lines(density(rgamma(1000, alpha5.2, beta5.2)), lty = 2, col = "red")
